...
def compute_asassn18fv():
    ...
    "R" : "I/297", 
    #"I" : "USNOB",
    "u" : "J/MNRAS/465/1807/carinaob",
    "g" : "skymapper",
    "r" : "skymapper",
    "i" : "J/MNRAS/465/1807/carinaob",
    }
    #ra = 159.069494
    #dec = -59.596710
    ra =159.06427566938083
    dec= -59.598258502684445
    
    p = Photometry()
    lfiles = glob.glob('/Users/nadiablago/Documents/Projects/ASASSN18fv/phot/lco/sdss/*fz')
    lfiles.sort()
    
    for imgfile in lfiles:
        
        filt = fitsutils.get_par(imgfile, "FILTER", ext=1)
        filt = p.filter_dic.get(filt, filt)
        ...
        if filt in survey_dic.keys():
            ...
            continue
        mjd = Time(fitsutils.get_par(imgfile, "DATE-OBS", ext=1)).mjd
        
        #Compute the zeropoint
        zp, zp_err, colorterm = p.get_zeropoint(imgfile, survey=survey, filt=filt, col_filt=p.col_dic[filt], plot=True)

        #Now finally get the positions of the transient and run aperture photometry on it
        #ra = fitsutils.get_par(imgfile, "RA", 1)
        #dec = fitsutils.get_par(imgfile, "DEC", 1)
        fwhm = fitsutils.get_par(imgfile, "FWHM", ext=1)
